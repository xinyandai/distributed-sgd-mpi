...
class ScalarCompressor(object):
    def __init__(self, size, shape, c_dim=1024, s=256, random=True):
        ...
        self.code_dtype = np.uint8 if self.s <= 2 ** 8 else (np.uint16 if self.s <= 2 ** 16 else np.uint32)
    ...
    def compress(self, vec):
        ...
        l = np.array(scaled_vec).clip(0, self.s-1).astype(dtype=self.code_dtype)
    ...
    def decompress(self, signature):
        ...
        scaled_vec = l.astype(dtype=np.float32) * (2 * signs.astype(dtype=np.float32) - 1)
